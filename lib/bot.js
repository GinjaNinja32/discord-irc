...
class Bot {
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      let displayUsername = username.replace(/./, match => match + "\uFEFF");
      ...
      if (this.isCommandMessage(text)) {
        ...
        if (text !== '') {
          var italicised = false; // whether we've seen an odd number of italics toggles so far
          text = text.replace(/\*{1,3}/g, match => {
                if(match.length == 3) { // ***
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const bold = "\x03**\x03"; // discord ignores the \x03, but treats it as a separator for the toggles
      const italic = "\x03*\x03";
      const underline = "__";
      let isBold = false; // whether there have been an odd number of bold toggles so far, so we can turn it off at the end (otherwise discord will treat the final toggle as a literal **)
      let isItalic = false; // ditto for italics
      let isUnderlined = false;
      let formatted = text
            .replace(/\*/g, "\\*") // change * into escaped \*
            .replace(/__/g, "\\_\\_") // change __ into escaped \_\_
            .replace(/\x03(\d{0,2},\d{0,2}|\d{0,2})/g, "") // strip color codes entirely
            .replace(/\x02|\x1D|\x0F|\x1F/g, match => { // bold/italic toggles and formatting reset
              if(match == "\x02") {
                isBold = !isBold;
                return bold;
              } else if(match == "\x1D") {
      ...
      var withMentions = formatted;
      const members = this.discord.users.getAll((_) => true);
      for(var i = 0; i < members.length; i++) {
        var user = members[i];
	withMentions = withMentions.replace("@" + user.username, user)
      }

      /*
      const withMentions = formatted.replace(/@[^\s]+\b/g, match => {
